...
bool Disk91_LoRaE5::persistConfig( // Store the LoRaWan configuration into module EEPROM for
                                   // restoring later
...
bool Disk91_LoRaE5::haveStoredConfig( // Returns true when a configuration has already been stored
                                      // in the E5 memory
) {
    bool             ret = true;
    struct s_persist p;
...
Disk91_LoRaE5::Disk91_LoRaE5(uint16_t atTimeoutMs, // Default timeout for the AT command processing
...
Disk91_LoRaE5::Disk91_LoRaE5(LOGSERIAL *logSerial // When set, the library debug is enabled
) {
    this->debugUart      = logSerial;
    this->runningCommand = false;
...
Disk91_LoRaE5::Disk91_LoRaE5(bool nothing // Strange complilation behavior, no param is not accepted
) {
    this->debugUart      = NULL;
    this->runningCommand = false;
    this->atTimeout      = __DSKLORAE5_DEFAULT_AT_TMOUT;
...
bool Disk91_LoRaE5::setup( // Setup the LoRaWAN stack with the stored credentials
    bool selfDC, // when true, the duty cycle management is not managed by the module but the user
                 // application
    ...
    if (p.version != 0x01)
        return false; // basic control, can be improved
    return this->setup(p.zone, p.deveui, p.appeui, p.appkey, selfDC, withADR);
}
...
bool Disk91_LoRaE5::setup( // Setup the LoRaWAN stack
    ...
    if (this->currentZone != zone) {
        ...
        if (zone == DSKLORAE5_ZONE_EU868) {
            ret &= sendATCommand("AT+DR=EU868", "+DR: EU868", "+DR: ERR", "", this->atTimeout,
                                 false, NULL);
            ret &= sendATCommand("AT+CH=3,867.1,0,5", "+CH: 3,8671", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=4,867.3,0,5", "+CH: 4,8673", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=5,867.5,0,5", "+CH: 5,8675", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=6,867.7,0,5", "+CH: 6,8677", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=7,867.9,0,5", "+CH: 7,8679", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            if (selfDC) {
                ret &=
                    sendATCommand("AT+LW=DC,OFF", "+LW: DC, OFF", "+LW: ERR", "", this->atTimeout,
                                  false, NULL); // manually managed to avoid conflicts
                ret &=
                    sendATCommand("AT+LW=JDC,OFF", "+LW: JDC, OFF", "+LW: ERR", "", this->atTimeout,
                                  false, NULL); // manually managed to avoid conflicts
            }
        } else if (zone == DSKLORAE5_ZONE_US915) {
            ret &= sendATCommand("AT+DR=US915", "+DR: US915", "+DR: ERR", "", this->atTimeout,
                                 false, NULL);
            // unvalidate the subband other than 2
            for (int i = 0; i < 72; i++) {
                if (i < 8 || i > 15) {
                    sprintf(_cmd, "AT+CH=%d,OFF", i);
                    ret &= sendATCommand(_cmd, "+CH: CH", "+CH: ERR", "", this->atTimeout, false,
                                         NULL);
                }
            }
            ...
            /*  According to https://github.com/helium/router
                ...
                924.4 MHz
            */
            ret &= sendATCommand("AT+DR=AS923", "+DR: AS923", "+DR: ERR", "", this->atTimeout,
                                 false, NULL);
            ret &= sendATCommand("AT+CH=0,923.2,0,5", "+CH: 0,9232", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=1,923.4,0,5", "+CH: 1,9234", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=2,923.6,0,5", "+CH: 2,9236", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=3,923.8,0,5", "+CH: 3,9238", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=4,924.0,0,5", "+CH: 4,9240", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=5,924.2,0,5", "+CH: 5,9242", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=6,924.4,0,5", "+CH: 6,9244", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+RXWIN2=923.2,DR2", "+RXWIN2: 9232", "+RXWIN2: ERR", "",
                                 this->atTimeout, false, NULL);
        } else if (zone == DSKLORAE5_ZONE_AS923_2) {
            ret &= sendATCommand("AT+DR=AS923", "+DR: AS923", "+DR: ERR", "", this->atTimeout,
                                 false, NULL);
            ret &= sendATCommand("AT+CH=0,921.4,0,5", "+CH: 0,9214", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=1,921.6,0,5", "+CH: 1,9216", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=2,921.8,0,5", "+CH: 2,9218", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=3,922.0,0,5", "+CH: 3,9220", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=4,922.2,0,5", "+CH: 4,9222", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=5,922.4,0,5", "+CH: 5,9224", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=6,922.6,0,5", "+CH: 6,9226", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+RXWIN2=921.4,DR2", "+RXWIN2: 9214", "+RXWIN2: ERR", "",
                                 this->atTimeout, false, NULL);
        } else if (zone == DSKLORAE5_ZONE_AS923_3) {
            ret &= sendATCommand("AT+DR=AS923", "+DR: AS923", "+DR: ERR", "", this->atTimeout,
                                 false, NULL);
            ret &= sendATCommand("AT+CH=0,916.6,0,5", "+CH: 0,9166", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=1,916.8,0,5", "+CH: 1,9168", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=2,917.0,0,5", "+CH: 2,9170", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=3,917.2,0,5", "+CH: 3,9172", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=4,917.4,0,5", "+CH: 4,9174", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=5,917.6,0,5", "+CH: 5,9176", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=6,917.8,0,5", "+CH: 6,9178", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+RXWIN2=916.6,DR2", "+RXWIN2: 9166", "+RXWIN2: ERR", "",
                                 this->atTimeout, false, NULL);
        } else if (zone == DSKLORAE5_ZONE_AS923_4) {
            ret &= sendATCommand("AT+DR=AS923", "+DR: AS923", "+DR: ERR", "", this->atTimeout,
                                 false, NULL);
            ret &= sendATCommand("AT+CH=0,917.3,0,5", "+CH: 0,9173", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=1,917.5,0,5", "+CH: 1,9175", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=2,917.7,0,5", "+CH: 2,9177", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=3,917.9,0,5", "+CH: 3,9179", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=4,918.1,0,5", "+CH: 4,9181", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=5,918.3,0,5", "+CH: 5,9183", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+CH=6,918.5,0,5", "+CH: 6,9185", "+CH: ERR", "",
                                 this->atTimeout, false, NULL);
            ret &= sendATCommand("AT+RXWIN2=917.3,DR2", "+RXWIN2: 9173", "+RXWIN2: ERR", "",
                                 this->atTimeout, false, NULL);
        } else if (zone == DSKLORAE5_ZONE_KR920) {
            ret &= sendATCommand("AT+DR=KR920", "+DR: KR920", "+DR: ERR", "", this->atTimeout,
                                 false, NULL);
        } else if (zone == DSKLORAE5_ZONE_IN865) {
            ret &= sendATCommand("AT+DR=IN865", "+DR: IN865", "+DR: ERR", "", this->atTimeout,
                                 false, NULL);
        } else if (zone == DSKLORAE5_ZONE_AU915) {
            ret &= sendATCommand("AT+DR=AU915", "+DR: AU915", "+DR: ERR", "", this->atTimeout,
                                 false, NULL);
            // unvalidate the subband other than 2
            for (int i = 0; i < 72; i++) {
                if (i < 8 || i > 15) {
    ...
    if (!withADR) {
        ret &=
            sendATCommand("AT+ADR=OFF", "+ADR: OFF", "+ADR: ON", "", this->atTimeout, false, NULL);
    } else {
        ret &=
            sendATCommand("AT+ADR=ON", "+ADR: ON", "+ADR: OFF", "", this->atTimeout, false, NULL);
    }

    // Setup Ids
    sprintf(_cmd, "AT+ID=DevEUI,%02X%02X%02X%02X%02X%02X%02X%02X", deveui[0], deveui[1], deveui[2],
            deveui[3], deveui[4], deveui[5], deveui[6], deveui[7]);
    ret &= sendATCommand(_cmd, "+ID: DevEui", "+ID: ERR", "", this->atTimeout, false, NULL);

    sprintf(_cmd, "AT+ID=AppEUI,%02X%02X%02X%02X%02X%02X%02X%02X", appeui[0], appeui[1], appeui[2],
            appeui[3], appeui[4], appeui[5], appeui[6], appeui[7]);
    ...
    sprintf(_cmd, "AT+KEY=APPKEY,%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",
            ...
            appkey[8], appkey[9], appkey[10], appkey[11], appkey[12], appkey[13], appkey[14],
            appkey[15]);
    ret &= sendATCommand(_cmd, "+KEY: APPKEY", "+KEY: ERR", "", this->atTimeout, false, NULL);
    ret &= sendATCommand("AT+MODE=LWOTAA", "+MODE: LWOTAA", "+MODE: ERR", "", this->atTimeout,
                         false, NULL);
    if (!ret) {
...
bool Disk91_LoRaE5::sendReceive_sync( // send a message on LoRaWan expert an ack at least,
                                      // eventually a downlink, return true when sent is a success
                                      // and expect a ack
    ...
    uint8_t retries // Number of retry, use DSKLORAE5_RT_UNCHANGED to keep the previous one. retry =
...
bool Disk91_LoRaE5::sendReceive( // send a message on LoRaWan, return true when sent is a success
    ...
    uint8_t  sz,                 // Size of the data, when 0 Join only is proceeded
    bool     acked,              // Ack / Downlink request
                // Downlink Callback, return size of downlink, 0 when ack, -1 for none, as a
                // parameter the buffer
    ...
    if (sz == 0 && this->hasJoined && !acked)
        return true; // basicaly nothing to do
    if (this->runningCommand)
        return false; // re-entering
    ...
    if (sf != DSKLORAE5_SF_UNCHANGED && this->lastSf != sf) {
        ...
        if (this->currentZone == DSKLORAE5_ZONE_EU868 ||
            this->currentZone == DSKLORAE5_ZONE_AS923_1 ||
            this->currentZone == DSKLORAE5_ZONE_AS923_2 ||
            this->currentZone == DSKLORAE5_ZONE_AS923_3 ||
            this->currentZone == DSKLORAE5_ZONE_AS923_4 ||
            this->currentZone == DSKLORAE5_ZONE_KR920 ||
            this->currentZone == DSKLORAE5_ZONE_IN865 ||
            this->currentZone == DSKLORAE5_ZONE_AU915) {
            // DR0 - SF12 / DR5 - SF7
            switch (sf) {
            case 7:
                retDr = sendATCommand("AT+DR=DR5", "+DR: ***** DR5", "+DR: ERR", "",
                                      this->atTimeout, false, NULL);
                break;
            case 8:
                retDr = sendATCommand("AT+DR=DR4", "+DR: ***** DR4", "+DR: ERR", "",
                                      this->atTimeout, false, NULL);
                break;
            case 9:
                retDr = sendATCommand("AT+DR=DR3", "+DR: ***** DR3", "+DR: ERR", "",
                                      this->atTimeout, false, NULL);
                break;
            case 10:
                retDr = sendATCommand("AT+DR=DR2", "+DR: ***** DR2", "+DR: ERR", "",
                                      this->atTimeout, false, NULL);
                break;
            case 11:
                retDr = sendATCommand("AT+DR=DR1", "+DR: ***** DR1", "+DR: ERR", "",
                                      this->atTimeout, false, NULL);
                break;
            case 12:
                retDr = sendATCommand("AT+DR=DR0", "+DR: ***** DR0", "+DR: ERR", "",
                                      this->atTimeout, false, NULL);
                break;
            default:
    ...
    if (!this->hasJoined) {
        ...
        this->estimatedDCMs +=
            100 * this->estimateTxDuration(this->lastSf, __DSKLORAE5_JOINREQ_PAYLOADSZ, 1);
        bool ret = true;
        if (async && sz == 0) {
            // We just want a join w/o uplink so we can make it async
            ret = sendATCommand("AT+JOIN", "+JOIN: Network joined", "+JOIN: Join failed",
                                "+JOIN: Done", __DSKLORAE5_JOIN_TIMEOUT, true, NULL);
            this->isJoining = true;
        } else {
            // We need to have s sync Join, case is simpler
            ret = sendATCommand("AT+JOIN", "+JOIN: Network joined", "+JOIN: Join failed",
                                "+JOIN: Done", __DSKLORAE5_JOIN_TIMEOUT, false, NULL);
            this->isJoining = false;
            if (ret) {
                this->hasJoined = true;
            }
    ...
    if (this->hasJoined && sz > 0) {
        ...
        if (acked) {
            ...
            this->_rxSize   = rxSize;
            this->_rxPort   = rxPort;
            ret             = sendATCommand(_cmd, "+CMSGHEX: Done", "", "",
                                            __DSKLORAE5_TX_TIMEOUT_BASE + txDuration +
                                                __DSKLORAE5_TX_TIMEOUT_ACK * (this->lastRetry + 1),
                                            async, processTx);
        } else {
            ret = sendATCommand(_cmd, "+MSGHEX: Done", "", "",
                                __DSKLORAE5_TX_TIMEOUT_BASE + txDuration, async, processTx);
        }
        if (!async && this->isBusy)
            ret = false;
        return ret;
    }
}
...
bool Disk91_LoRaE5::processTx(Disk91_LoRaE5 *wrap) {
    if (startsWith(wrap->bufResponse, "+CMSGHEX: RXWIN*, RSSI") ||
        startsWith(wrap->bufResponse, "+MSGHEX: RXWIN*, RSSI")) {
        ...
        wrap->hasAcked = true;
               ...
               startsWith(wrap->bufResponse, "+MSGHEX: Done")) {
        uint32_t elapsedTime = millis() - wrap->startTime;
        if (wrap->hasAcked) {
            // will see later if we want to use this
            uint8_t retries =
                elapsedTime / wrap->estimateTxDuration(wrap->lastSf, wrap->lastSize,
                                                       1); // really approximative approach
        } else {
        ...
        wrap->currentSeqId = (wrap->currentSeqId + 1) & 0xFFFF;
               ...
               startsWith(wrap->bufResponse, "+MSGHEX: FPEND")) {
        // downlink pending
        wrap->downlinkPending = true;
    } else if (startsWith(wrap->bufResponse, "+CMSGHEX: PORT: *; RX: ") ||
               startsWith(wrap->bufResponse, "+MSGHEX: PORT: *; RX: ")) {
        // downlink content
    ...
    } else if (startsWith(wrap->bufResponse, "+CMSGHEX: LoRaWAN modem is busy") ||
               startsWith(wrap->bufResponse, "+MSGHEX: LoRaWAN modem is busy")) {
        wrap->isBusy = true;
    } else {
        // unprocessed lines
    }
...
uint32_t Disk91_LoRaE5::estimateTxDuration(uint8_t sf, uint8_t payloadSz, uint8_t retries) {
    if (this->currentZone == DSKLORAE5_ZONE_EU868) {
        ...
        switch (sf) {
        case 7:
            return (retries * (4630 + payloadSz * 225)) / 100;
        case 8:
            return (retries * (9270 + payloadSz * 256)) / 100;
        case 9:
            return (retries * (16490 + payloadSz * 450)) / 100;
        case 10:
            return (retries * (32970 + payloadSz * 819)) / 100;
        case 11:
            return (retries * (65950 + payloadSz * 1802)) / 100;
        default:
        case 12:
            return (retries * (115510 + payloadSz * 3277)) / 100;
        }
    } else {
 ...
 */
bool Disk91_LoRaE5::sendATCommand(const char *cmd, const char *okResp, const char *errResp,
                                  const char *ending, uint32_t timeoutMs, bool async,
    ...
    while ((this->isHwSerial && this->e5Uart->available() > 0) ||
        ...
        if ((c == '\0' || c == '\r' || c == '\n')) {
            if (this->respIndex > 0) {
                ...
                if (strlen(this->bufErrResp) > 0 &&
                    startsWith(this->bufResponse, this->bufErrResp)) {
                    // Error String found
