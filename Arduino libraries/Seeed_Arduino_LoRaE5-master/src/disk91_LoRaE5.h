...
#define DSKLORAE5_SWSERIAL_PINS                                                                    \
    0x11 // LoRa-E5 on a software serial to be created by library with pins
...
#define __DSKLORAE5_TX_TIMEOUT_BASE                                                                \
    3200                                // Specific timeout for Tx w/o ack ( tx time will be added )
#define __DSKLORAE5_TX_TIMEOUT_ACK 3500 // Time to add for Ack / Downlink frames
#define __DSKLORAE5_JOINREQ_PAYLOADSZ                                                              \
    24 // Estimation of a Join Req frame as a normal frame with a given payload of 24bytes
...
class Disk91_LoRaE5 {
  ...
  protected:
    ...
    uint16_t currentSeqId;     // Next communication estimated SeqId (the module does not allow to
                               // access it, so ... a bit a mess)
    ...
    uint32_t estimatedDCMs;    // Next communication autorization for when DC management is not
                               // delegated to module (DC zone only)
    ...
    bool sendATCommand(      // Send an AT command to the E5 module
        ...
        const char *okResp,  // expected char string to be find in response to consider the AT
                             // execution a success, * is a joker
        const char *errResp, // expected char string to be find in response to consider the AT
                             // execution a failure, * is a joker, null is err if not ok
        const char *ending,  // expected char string to be find in response to consider end of AT
                             // command, null means timeout (slower)
        uint32_t timeoutMs,  // timeout for getting a response  from E5
        bool     async,      // async processing with the E5 loop.
        bool (*lineProcessing)(
            Disk91_LoRaE5 *) // callback function to process each line of the E5 response
    );
    ...
    static bool processTx(Disk91_LoRaE5 *wrap); // Process each of the line recaived from e5 when a
                                                // tx request has been made
    ...
    bool sendReceive(   // send a message on LoRaWan, return true when sent is a success
        ...
        uint8_t
            *rxSize, // uint8_t containing the rxBuffer size and returning the downlink message size
        ...
        uint8_t  retries, // Number of retry, use DSKLORAE5_RT_UNCHANGED to keep the previous one.
                          // retry = 0 means 1 uplink, no retry
        bool async        // When true, the processing is made synchronously
    );

  public:
    Disk91_LoRaE5(bool nothing = false // if anyone can explain me why w/o param the constructer
                                       // generate compilation error ?!?
    );

    Disk91_LoRaE5(uint16_t   atTimeoutMs,     // Default timeout in Ms for AT command execution
                  LOGSERIAL *logSerial = NULL // When set, the library debug is enabled
    );

    Disk91_LoRaE5(LOGSERIAL *logSerial = NULL // When set, the library debug is enabled
    );
    ...
    bool begin(
        ...
        int16_t         ssRxPort =
            -1, // for SWSERIAL_PINS, specify the RX & TX Pin, the SSerial will be initialized
        int16_t ssTxPort = -1);
    ...
    bool
    persistConfig( // Store the LoRaWan configuration into module EEPROM for restoring later
    ...
    bool haveStoredConfig();  // Returns true when a configuration has already been stored in the E5
                              // memory
    bool clearStoredConfig(); // Purge the stored configuration for E5

    bool sendReceive_sync( // send a message on LoRaWan expert an ack at least, eventually a
                           // downlink, return true when sent is a success and expect a ack
        uint8_t  port,     // LoRaWan port
        uint8_t *data,     // Data / payload to be transmitted
        uint8_t  sz,       // Size of the data, when 0 Join only is proceeded
        uint8_t *rxBuffer, // Downlink buffer, make sure it will be large enought, no verification
        ...
        uint8_t retries = 0 // Number of retry, use DSKLORAE5_RT_UNCHANGED to keep the previous one.
                            // retry = 0 means 1 uplink, no retry
    );
    ...
    bool join_sync( // Join only (join is automatically executed on send but you can decide to join
                    // sepratly)
        uint8_t sf  = 9, // Spread Factor , use DSKLORAE5_SF_UNCHANGED to keep the previous one
        uint8_t pwr = 14 // Transmission power, use DSKLORAE5_DW_UNCHANGED to keep the previous one
    );
    ...
    bool
    isDownlinkPending(); // return true when the previous uplink got a downlink pending response
    bool    isDownlinkReceived(); // return true when the previous uplink got a downlink response
    int16_t getRssi(); // return last Ack RSSI when the previous uplink has been confirmed as
                       // received or DSKLORAE5_INVALID_RSSI
    int16_t getSnr(); // return last Ack SNR when the previous uplink has been confirmed as received
                      // or DSKLORAE5_INVALID_SNR
